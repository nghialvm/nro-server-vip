...
public class EventPanel extends JPanel {
    ...
    public void doiSuKienVaApDungNgay(List<Integer> eventIds) {
        log("Đang tiến hành lưu cấu hình vào data/config/active_event.txt...");
        saveEventConfig(eventIds);
    ...
    private void saveEventConfig(List<Integer> eventIds) {
        try (PrintWriter pw = new PrintWriter(new FileWriter("data/config/active_event.txt"))) {
            ...
            Logger.error("Lỗi lưu file data/config/active_event.txt: " + e.getMessage());
        }
    }

    public List<Integer> loadEventConfig() {
        List<Integer> ids = new ArrayList<>();
        File f = new File("data/config/active_event.txt");
        if (!f.exists()) {
            ids.add(11);
            return ids;
        }
